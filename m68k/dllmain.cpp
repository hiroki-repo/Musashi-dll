...
extern "C" {
    ...
    void cpu_write_byte(unsigned int addr, int value) { if (databussize == 0) { m68kmemaccess(addr, value, 0); } else { m68kmemaccess(addr, value, 0 | (1 << 8)); } }
    void cpu_write_word(unsigned int addr, int value) { if (databussize == 0) { m68kmemaccess(addr, swapendianess16(value), 0); } else if (databussize == 1) { m68kmemaccess(addr, ((value >> (8 * 1)) & 0xFF), 0 | (1 << 8)); m68kmemaccess(addr + 1, ((value >> (8 * 0)) & 0xFF), 0 | (1 << 8)); } else if (databussize >= 2) { m68kmemaccess(addr, swapendianess16(value), 0 | (2 << 8)); } }
    void cpu_write_long(unsigned int addr, int value) { if (databussize == 0) { m68kmemaccess(addr, swapendianess32(value), 0); } else if (databussize == 1) { m68kmemaccess(addr, ((value >> (8 * 3)) & 0xFF), 0 | (1 << 8)); m68kmemaccess(addr + 1, ((value >> (8 * 2)) & 0xFF), 0 | (1 << 8)); m68kmemaccess(addr + 2, ((value >> (8 * 1)) & 0xFF), 0 | (1 << 8)); m68kmemaccess(addr + 3, ((value >> (8 * 0)) & 0xFF), 0 | (1 << 8)); } else if (databussize == 2) { m68kmemaccess(addr, swapendianess16((value >> (16 * 1)) & 0xFFFF), 0 | (2 << 8)); m68kmemaccess(addr + 2, swapendianess16((value >> (16 * 0)) & 0xFFFF), 0 | (2 << 8)); } else if (databussize >= 4) { m68kmemaccess(addr, swapendianess32(value), 0 | (4 << 8)); } }
